...
class Gtk4Dialogs {
  ...
  /** The translations that GTK 4 labels its own buttons from. */
  private final String GTK_DOMAIN = "gtk40";
        ...
        response ->
            completion.complete(
                response == Gtk.RESPONSE_ACCEPT
                    ? Gtk.fileChooserPaths(chooser).stream().map(Path::of).toList()
                    : List.of()));
  }
  ...
  void save(
    ...
    present(
        ...
        response ->
            completion.complete(
                response == Gtk.RESPONSE_ACCEPT
                    ? Gtk.fileChooserPaths(chooser).stream().findFirst().map(Path::of)
                    : Optional.empty()));
  }

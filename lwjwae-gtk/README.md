...
The provider [`GtkApplicationBackendProvider`](src/main/java/dev/ivchenko/lwjwae/gtk/GtkApplicationBackendProvider.java) registers under the name `gtk3-webkit2gtk-4.1`. It
...
| Class                                                                                       | Role                                                                                        |
|---------------------------------------------------------------------------------------------|---------------------------------------------------------------------------------------------|
| [`GtkApplicationBackend`](src/main/java/dev/ivchenko/lwjwae/gtk/GtkApplicationBackend.java) | The window. Forwards every call to the GTK thread.                                          |
...
`new GtkApplicationBackend(parameters)` runs the following on the GTK thread and returns when it's
done:
...
5. Creates the `WebKitWebView` with that content manager and adds it to the window.
6. Connects `destroy` on the window and `load-changed`, `load-failed`, and `context-menu` on the
   view. Every handler is a static upcall stub; the user data is the ID of the backend in a
   [`CallbackRegistry`](../lwjwae-core/src/main/java/dev/ivchenko/lwjwae/foreign/CallbackRegistry.java).
7. Calls `installBridge()`, which adds the bridge runtime as a user script at document start.

The window stays hidden until `show()`, which calls `gtk_widget_show_all`.

## Callbacks

Every signal handler follows the same shape: look up the backend by user data, do the work, catch
...
| `destroy`                 | `onDestroy`       | Unregisters the backend, clears the handles, and calls `markClosed()`.                                                                                                                                                                |

## Serving resources

The `app://local/PATH` scheme is answered by `onResourceRequest`, which isn't tied to a window:
...
synchronously, so `onDestroy` runs before `close()` returns, and every thread blocked in `run()` is
released. Closing the window with the title bar button takes the same path. `close()` is
idempotent.
